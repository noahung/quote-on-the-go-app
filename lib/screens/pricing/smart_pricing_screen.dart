...
import '../expenses/log_expense_screen.dart';
...
class _SmartPricingScreenState extends ConsumerState<SmartPricingScreen>
  ...
  Future<void> _applyNewPrice(BuildContext context, String serviceId,
      String serviceName, double newPrice) async {
    ...
    try {
      ...
      scaffoldMessenger.showSnackBar(
        SnackBar(
          content: Text(
              'Updated $serviceName price to £${newPrice.toStringAsFixed(0)}!'),
  ...
  Widget _buildOptimizerTab(
      BuildContext context, SemanticColors colors, bool isDark) {
    final suggestionsAsync =
        ref.watch(pricingSuggestionsProvider(_selectedServiceId));
    ...
    return suggestionsAsync.when(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              OutlinedButton(
                onPressed: () => ref
                    .invalidate(pricingSuggestionsProvider(_selectedServiceId)),
      ...
      data: (suggestions) {
        ...
        if (suggestion == null) {
          return const Center(
              child: Text('No suggestion for this service.',
  ...
  Widget _buildSuggestionCard(BuildContext context,
      Map<String, dynamic> suggestion, SemanticColors colors, bool isDark) {
    final serviceName = suggestion['serviceName'] as String? ?? 'Unknown';
    final suggestedPrice =
        (suggestion['suggestedPrice'] as num?)?.toDouble() ?? 0;
    ...
    final historical =
        suggestion['historicalPerformance'] as Map<String, dynamic>?;
    final revenueImpact = historical?['revenueImpact'] as Map<String, dynamic>?;
    final riskLevel = revenueImpact?['riskLevel'] as String? ?? 'low';
    final projectedIncrease =
        (revenueImpact?['projectedIncrease'] as num?)?.toDouble() ?? 0;
              ...
              children: [
                  ...
                  children: [
                    ...
                    const Padding(
                      ...
                      child: Icon(Icons.arrow_forward,
                          color: Color(0xFFF4781F), size: 24),
                    ),
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color:
                                    (isIncrease ? colors.success : colors.error)
                ...
                FilledButton(
                  onPressed: _isApplying
                      ? null
                      : () => _applyNewPrice(
                          context, serviceId, serviceName, suggestedPrice),
                  ...
                  child: _isApplying
                      ...
                      : const Text('Apply Suggested Price',
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.error_outline, size: 48, color: Colors.grey),
              const SizedBox(height: 12),
              Text('Failed to load bundles: $err',
                  textAlign: TextAlign.center,
                  style: const TextStyle(fontSize: 13)),
              const SizedBox(height: 16),
              OutlinedButton(
  ...
  Widget _buildBundleCard(
      SemanticColors colors, bool isDark, Map<String, dynamic> bundle) {
    ...
    final discountPercentage =
        (bundle['discountPercentage'] as num?)?.toDouble() ?? 0;
    final acceptanceRate =
        (bundle['expectedAcceptanceRate'] as num?)?.toDouble() ?? 0;
          ...
          children: [
            ...
            if (services.isNotEmpty) ...[
              ...
              ...services.map((s) {
                ...
                final individual =
                    (svc['individualPrice'] as num?)?.toDouble() ?? 0;
                final bundlePrice =
                    (svc['bundlePrice'] as num?)?.toDouble() ?? 0;
                return Padding(
            ...
            Row(
              ...
              children: [
                ...
                OutlinedButton(
                  onPressed: !totalPrice.isFinite || totalPrice < 0
                      ? null
  ...
  Widget _buildDiscountCard(BuildContext context, dynamic quotation,
      SemanticColors colors, bool isDark) {
    final recommendationAsync =
        ref.watch(discountRecommendationProvider(quotation.id));
          ...
          children: [
            ...
            recommendationAsync.when(
              ...
              data: (rec) {
                ...
                final discountValue =
                    (rec['discountValue'] as num?)?.toDouble() ?? 0;
                final reasoning = rec['reasoning'] as String? ?? '';
                final currentProb =
                    (rec['currentWinProbability'] as num?)?.toDouble() ?? 0;
                final projectedProb =
                    (rec['projectedWinProbability'] as num?)?.toDouble() ?? 0;
                ...
                return Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Text(
                            'Win prob: ${(currentProb * 100).toStringAsFixed(0)}%',
                            style: const TextStyle(
                  ...
                  Row(
                    children: [
                      const Icon(Icons.receipt_long,
                          color: Color(0xFFF4781F), size: 28),
                      ...
                      onPressed: () => Navigator.of(context).push(
                          MaterialPageRoute<void>(
                              builder: (_) => LogExpenseScreen(
                                  receiptData: _result,
                                  initialReceipt: _receipt))),
                      child: const Text('Review and save expense')),
                  const SizedBox(height: 12),
                  Row(
                    children: [
                      Expanded(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text('AI Receipt Analysis',
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            const Text(
            ...
            if (_error != null) ...[
              const SizedBox(height: 16),
              Text(_error!,
                  style: TextStyle(fontSize: 12, color: widget.colors.error)),
            ],
          ],
        ),
      ),
